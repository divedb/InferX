...
| R9 | **CUDA graph replay computes the wrong logits — always** | **High — open** | Not a concurrency bug and not intermittent, as previously recorded here. `EngineTest.GraphedAndUngraphedLogitsAgreeForABatch` runs one decode batch launch-by-launch and then replays a graph captured for the same shape: on a *single* sequence the logits differ by ~19 from the first vocabulary entry. A control re-running the batch with nothing captured in between is bit-identical, so the graph is the cause. It hid since M3 because every graph test compared sampled tokens, not logits, and the argmax usually survives an error that size; when it does not, the sequence derails — which is the garbage that was reported as a concurrency failure. Sweeping the capture probe's position changes the error (2.4 at position 0, 19.9 at position 5, where the argmax flips), so the graph has baked probe-derived values instead of reading them per replay. FlashInfer's plan is the prime suspect: `Plan` computes host-side values that `Run` passes as kernel arguments, and capture freezes them — which is precisely what `graph_safe` was supposed to prevent. **Consequence: the 1.05x attributed to CUDA graphs in the decode-step progression was measuring a wrong computation and should not be counted.** Also ruled out: memory safety (memcheck clean on a reproducing run) and buffer reallocation after capture (instrumented; none occurs). `capture_graphs` stays off; `--cuda-graphs` opts in. The known-failing test is left red on purpose. |
