...
prefix caching on, `--max-num-seqs` 8 in both, 4096-token context. Greedy
...
- **FP8 activation quantization ran in a single CUDA block.** One block is the
  ...
  a whole step's activations went through one SM. Serving with
  `--quantization fp8` alone —
