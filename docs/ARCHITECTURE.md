...
| **M7** | **In progress.** `Communicator`, `SingleRankComm`, and deterministic `HostSimComm` are built, including reusable-rendezvous, BF16, mismatch, TP=4, and CUDA-staging collective tests. Dense Qwen2 now shards Q/K/V and gate/up by output, O/down by input, and KV heads per rank; O and down are followed by all-reduce. The real Qwen2.5-3B checkpoint produces matching TP=1 and simulated TP=2 logits on one GPU. TP=2 is this checkpoint's maximum pure head shard because it has two KV heads; TP=4 collective behavior is tested independently. Embeddings, norms, and LM head remain replicated. Graph capture is deliberately rejected for HostSim TP because its collective stages through the host. Next: paged TP differential coverage, rank orchestration, then NCCL when multi-GPU hardware exists. | G5 as far as this hardware allows |
