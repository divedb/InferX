...
| **M7** | **In progress.** `Communicator`, `SingleRankComm`, and deterministic `HostSimComm` are built, including reusable-rendezvous, BF16, mismatch, TP=4, and CUDA-staging collective tests. Dense Qwen2 shards Q/K/V and gate/up by output, O/down by input, and KV heads per rank; O and down are followed by all-reduce. The real Qwen2.5-3B checkpoint produces matching TP=1 and simulated TP=2 logits on one GPU in full recompute, paged prefill, and the following incremental decode step; each rank's KV pool is half-sized. TP=2 is this checkpoint's maximum pure head shard because it has two KV heads; TP=4 collective behavior is tested independently. Embeddings, norms, and LM head remain replicated. Graph capture is deliberately rejected for HostSim TP because its collective stages through the host. Next: server rank orchestration, then NCCL and graph-capable collectives when multi-GPU hardware exists. | G5 as far as this hardware allows |
