...
| R9 | CUDA graph capture at startup is not trustworthy | Medium — **open, and intermittent** | Concurrent decode under `--cuda-graphs` returned fluent nonsense. Two real defects were found and fixed along the way: the capture probe stood at position 0, so the index buffers FlashInfer reads were sized for a one-block batch and the first real request reallocated them out from under every captured graph; and capture ran before any realistic batch had executed. With both fixed, three trials at the configuration that used to fail now match the un-graphed output exactly. **That is not a fix.** The failure was never made deterministic — the same binary and config produced garbage on one run and correct output on another — so the changes may only have shifted the odds. `ServerGraphsTest` asserts the right property (graphed output must equal un-graphed, and repeated runs must agree) but does not reproduce the failure even with the fixes reverted, so it is a guard rather than a regression test. `capture_graphs` stays off; `--cuda-graphs` opts in. |
