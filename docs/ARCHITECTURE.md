...
| **M7** | ✅ **Tensor parallelism at the single-GPU validation scope.** `Communicator`, `SingleRankComm`, and deterministic `HostSimComm` include reusable-rendezvous, BF16, mismatch, TP=4, and CUDA-staging collective tests. Dense Qwen2 shards Q/K/V and gate/up by output, O/down by input, and KV heads per rank; O and down are followed by all-reduce. Exact shard reconstruction is tested. The real Qwen2.5-3B checkpoint produces matching TP=1 and simulated TP=2 logits on one GPU in full recompute, paged prefill, and the following incremental decode step; each rank's KV pool is half-sized. TP=2 is this checkpoint's maximum pure head shard because it has two KV heads; TP=4 collective behavior is tested independently. Embeddings, norms, and LM head remain replicated. HostSim graph capture is rejected because its collective stages through the host. **Deferred by design:** server multi-process/rank deployment, NCCL, performance tuning, and graph-capable device collectives require multi-GPU hardware. | G5 as far as this hardware allows — **met** |
