...
| R9 | CUDA graph capture is not trustworthy for concurrent decode | Medium — **open, mechanism unknown** | Concurrent decode under `--cuda-graphs` returns fluent nonsense; single-sequence decode is correct. **Ruled out by experiment, not by argument:** stale KV (zeroing the cache on retire changed nothing); FlashInfer (forcing the reference kernel changed nothing); the in-graph sampler (host sampling changed nothing); multi-shape interference (capturing one shape failed identically); memory safety — `compute-sanitizer --tool=memcheck` reports **0 errors** on a run that reproduces the garbage; and stranded pointers — instrumenting every index-buffer reallocation shows **none happens after any graph is captured**, so the captured pointers are stable. That last result also disproves the mechanism claimed for the two changes in `eee7c49`: they are defensible hardening (buffers should reach full size before capture, and shapes should be executed before being recorded) but they are *not* the fix, and R9 is not resolved. `capture_graphs` stays off; `--cuda-graphs` opts in. Next: log which graph is replayed per step and diff a graphed against an ungraphed step at the tensor level, since the fault is evidently in what the replay computes rather than in what it touches. |
