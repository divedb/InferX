...
| **M7** | **In progress.** `Communicator`, `SingleRankComm`, and deterministic `HostSimComm` are built, including reusable-rendezvous, BF16, mismatch, TP=4, and CUDA-staging collective tests. Collectives carry an opaque execution stream; host simulation accepts CPU tensors or stages CUDA tensors for one-GPU differential testing. Host checkpoint tensors have exact shard/reconstruct utilities for arbitrary byte-aligned axes, with column-, row-, higher-rank, divisibility, and packed-int4 tests. Next: dense Qwen2 column/row parallel projections and TP=1 vs simulated TP=4 logits. Real NCCL performance remains deferred to multi-GPU hardware. | G5 as far as this hardware allows |
