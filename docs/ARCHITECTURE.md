...
| R8 | **KV state leaks between sequences** | **High — open** | Found at M4. Identical greedy requests return different output, alternating in step with block recycling, so a sequence's result depends on what ran before it. `last_page_len` and the CSR block table are both correct, so this is not yet diagnosed. Reproduced by `ServerTest.IdenticalRequestsProduceIdenticalOutput`, which is a known failure kept red on purpose |
| R9 | Multi-shape CUDA graph capture is unsound | Medium — open | Preparing a larger batch grows on-demand buffers, stranding addresses already recorded by graphs captured for smaller shapes. Capturing largest-first fixes single-sequence decode; concurrent decode is still wrong, so `capture_graphs` ships off and `--cuda-graphs` opts in |
...
| **M4** | ⚠️ **HTTP server, tokenizer, SSE streaming, OpenAI API.** Delivered on cpp-httplib, *not* Beast (§9/§5.1), and on our own byte-level BPE, *not* tokenizer FFI (R7) | End-to-end serving — `inferx-serve` answers. Blocked on a KV bug: identical greedy requests return different output (see R8) |
