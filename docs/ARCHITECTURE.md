...
| R9 | Multi-shape CUDA graph capture is unsound | Medium — open | Preparing a larger batch grows on-demand buffers, stranding addresses already recorded by graphs captured for smaller shapes. Capturing largest-first fixes single-sequence decode; concurrent decode is still wrong *after* R8 was fixed, so the two are genuinely separate. `capture_graphs` ships off and `--cuda-graphs` opts in |
