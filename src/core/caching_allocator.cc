...
CachingAllocator::CachingAllocator(std::byte* base, size_t capacity, DeviceId device,
                                   size_t alignment)
    : device_(device) {
...
void CachingAllocator::RemoveFree(Block* b) { free_blocks_.erase(FreeKey{b->size, b->offset}); }
...
StatusOr<void*> CachingAllocator::Allocate(size_t bytes, size_t alignment) {
  ...
  if (found.block == nullptr) {
    ...
    return ResourceExhaustedError("workspace allocator on ", device_.ToString(),
                                  " cannot satisfy ", need, " bytes aligned to ", alignment,
                                  ": ", s.capacity - s.in_use, " free across ",
                                  s.free_block_count, " blocks, largest ", s.largest_free_block,
                                  " bytes (fragmentation ", s.Fragmentation(), ")");
  }
...
Status CachingAllocator::ValidateAllocateArgs(size_t bytes, size_t alignment) const {
  if (base_ == nullptr) {
  ...
  if (alignment == 0 || !std::has_single_bit(alignment)) {
    return InvalidArgumentError("alignment ", alignment, " is not a non-zero power of two");
  }

  return OkStatus();
}

CachingAllocator::FoundBlock CachingAllocator::TakeFreeBlock(size_t need, size_t alignment) {
  ...
  return alignment <= alignment_ ? TakeBestFit(need) : TakeOverAligned(need, alignment);
}
...
CachingAllocator::FoundBlock CachingAllocator::TakeOverAligned(size_t need, size_t alignment) {
  ...
  for (auto it = free_blocks_.lower_bound(FreeKey{need, 0}); it != free_blocks_.end(); ++it) {
...
StatusOr<Tensor> CachingAllocator::AllocateTensor(const TensorSpec& spec) {
  ...
  return Tensor::FromBlob(p, spec.GetDataType(), spec.GetShape(), device_);
}
...
CachingAllocator::Stats CachingAllocator::GetStats() const {
  ...
  s.largest_free_block = free_blocks_.empty() ? 0 : std::prev(free_blocks_.end())->first.first;
