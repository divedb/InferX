...
StatusOr<void*> BumpArena::Allocate(size_t bytes, size_t alignment) {
  ...
  if (alignment == 0 || !std::has_single_bit(alignment)) {
    return InvalidArgumentError("alignment ", alignment, " is not a power of two");
  }
  ...
  if (!new_offset || *new_offset > capacity_) {
    return ResourceExhaustedError("BumpArena exhausted on ", device_.ToString(), ": requested ",
                                  bytes, " bytes (aligned to ", alignment, ") with ",
                                  capacity_ - offset_, " of ", capacity_, " remaining");
  }
...
StatusOr<Tensor> BumpArena::AllocateTensor(DataType dtype, const Shape& shape) {
  ...
  return Tensor::FromBlob(p, dtype, shape, device_);
}
