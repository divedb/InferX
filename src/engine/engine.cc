  ...
  void Deliver(const TokenDelta& delta) {
    ...
    if (!state.stop.empty()) {
      ...
      if (at != std::string::npos) {
        ...
        // The scheduler retires the request on the next shared model step.
...
StatusOr<std::unique_ptr<Engine>> Engine::Create(const EngineConfig& config) {
  ...
  // Keep checkpoint interpretation outside the serving loop. The factory owns
  // architecture-specific construction and returns the common runner contract.
