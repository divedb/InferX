...
folly::coro::Task<void> CompletionHandler::Handle(
  ...
  // The stream's `text_offset` entries keep counting from wherever the
  // previous chunk stopped, and an echoed prompt occupies the first bytes.
  size_t stream_offset = parsed->sampling.echo ? parsed->prompt.size() : 0;
  ...
  if (parsed->sampling.stream) {
    ...
    // Echo means the client wants its prompt back on the same channel as the
    // generation, so it leads the stream as an ordinary content chunk.
    if (parsed->sampling.echo) {
      Status echoed = co_await sse.Event(
          ::inferx::api::CompletionChunkJson(submitted->request_id,
                                             parsed->model, parsed->prompt,
      ...
      if (!event.text_delta.empty() || !event.logprobs.empty()) {
        ...
        const Status written = co_await sse.Event(
            ::inferx::api::CompletionChunkJson(
                ...
                nullptr, created, sampled, logprobs, stream_offset),
            cancellation);
        if (!written.ok()) {
          (void)co_await requests_->Cancel(
              submitted->request_id,
        ...
        for (const auto& token : decoded.tokens) {
          stream_offset += token.token.size();
        }
      }
    } else {
  ...
  std::vector<::inferx::api::CompletionChoice> choices;
  choices.reserve(outcomes.size());
  for (size_t index = 0; index < outcomes.size(); ++index) {
    ::inferx::api::CompletionChoice out_choice;
    out_choice.text = parsed->sampling.echo
                          ? parsed->prompt + outcomes[index].text
                          : std::move(outcomes[index].text);
    ...
    if (parsed->sampling.want_logprobs) {
      ...
      // Reported tokens start after the echoed prompt, whose bytes lead the
      // choice's text.
      out_choice.text_offset = parsed->sampling.echo ? parsed->prompt.size() : 0;
    }
