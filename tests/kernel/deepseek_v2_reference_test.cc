...
bool LoadDecodeReference(DecodeReference* out, std::string* why) {
  ...
  const bool ok = std::fread(out->prompt.data(), sizeof(int32_t), prompt, f) ==
                      prompt &&
                  std::fread(out->generated.data(), sizeof(int32_t), generated,
                             f) == generated;
  std::fclose(f);
  if (!ok) {
    *why = path + ": truncated body";
...
TEST_F(DeepseekV2ReferenceTest,
  ...
  auto model = DeepseekV2Model::Load(CheckpointDir());
  if (!model.ok()) {
  ...
  while (generated < reference.generated.size()) {
    ...
    ASSERT_TRUE(model->Step(batch, &logits).ok())
        << "model step " << generated;

    std::vector<int32_t> sampled;
    const int64_t vocab = model->config().vocab_size;
    ASSERT_EQ(logits.size(), batch.logits_indices.size() *
    ...
    for (size_t row = 0; row < batch.logits_indices.size(); ++row) {
      ...
      if (token != ref_token) {
        ...
        ASSERT_LE(ref_margin, kTieTolerance)
            << "first cached greedy divergence at generated token "
            << generated << " (position "
            << reference.prompt.size() + generated << "): cached picked "
            << token << " over the reference's " << ref_token
            << " (cached margin " << cached_margin
