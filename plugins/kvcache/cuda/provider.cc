// Session lifecycle, negotiation, object table, control plane, and the
// GPU/host tiering engine of the CUDA provider.
...
// Rolls an unfinished transaction back. Storage of blocks that pending
// transfers still reference is reclaimed when those transfers finish; the
// transaction itself becomes unusable immediately.
void AbortLocked(Session& session, const TxPtr& tx) {
  if (tx->state == KVC_COMMITTED || tx->state == KVC_WRITE_ABORTED) return;
  session.gpu_reserved -= tx->reserved_remainder;
  tx->reserved_remainder = 0;
  session.stats_tx_aborted.fetch_add(1, std::memory_order_relaxed);
  for (const auto& block : tx->blocks) {
    session.reserved_keys.erase(Session::KeyId(block->record.key));
    // Blocks held only by the transaction are reclaimed here so their
    // destruction under this lock stays trivial.
    if (block.use_count() == 1) session.ReclaimLocked(block);
  }
...
// ---------------------------------------------------------------------------
// Slot pools and block storage
// ---------------------------------------------------------------------------

SlotPool::~SlotPool() {
  cudaSetDevice(device);
  for (void* slot : free_slots) cudaFree(slot);
}
...
void* SlotPool::Take() {
  ...
  cudaSetDevice(device);
  void* slot = nullptr;
  if (cudaMalloc(&slot, slot_bytes) != cudaSuccess) return nullptr;
  ++live;
  ++total;
...
void SlotPool::Give(void* slot) {
  std::lock_guard<std::mutex> lock(mutex);
  free_slots.push_back(slot);
  --live;
}

uint64_t SlotPool::LiveBytes() {
  std::lock_guard<std::mutex> lock(mutex);
  return live * slot_bytes;
}

void Block::ReclaimStorage() {
  if (gpu) {
    pool->Give(gpu);
    gpu = nullptr;
    pool.reset();
  ...
  if (!host.empty()) {
    session->host_bytes.fetch_sub(host.size(), std::memory_order_relaxed);
    host.clear();
    host.shrink_to_fit();
  }
}
...
SlotPool* Session::PoolForLocked(uint64_t slot_bytes) {
  auto it = pools.find(slot_bytes);
  if (it != pools.end()) return it->second.get();
  auto pool = std::make_shared<SlotPool>(slot_bytes);
  pool->device = device_ordinal;
  SlotPool* raw = pool.get();
  pools.emplace(slot_bytes, std::move(pool));
  return raw;
}

Session::~Session() {
  // No provider objects are alive here (shutdown drained them), so dropping
  // the last block references is safe. Reclaim first so the destructors,
  // which would lock this session, stay trivial.
  for (auto& [_, block] : index) ReclaimLocked(block);
  index.clear();
  for (auto& [_, tx] : transactions) {
    for (auto& block : tx->blocks) ReclaimLocked(block);
    tx->blocks.clear();
  }
  transactions.clear();
  pools.clear();
...
uint64_t Session::GpuUsedLocked() {
  uint64_t used = gpu_reserved;
  for (auto& [_, pool] : pools) used += pool->LiveBytes();
  return used;
}

KvcStatus Session::MakeHostRoomLocked(uint64_t bytes) {
  while (host_bytes.load(std::memory_order_relaxed) + bytes > host_budget) {
    // Drop the least-recently-used committed, unpinned host block.
    BlockPtr victim;
    for (auto& [_, block] : index) {
      if (block->host.empty() || block.use_count() != 1 || block->tx_private) continue;
      if (!victim || block->last_use < victim->last_use) victim = block;
    }
    if (!victim) return ErrorStatus(KVC_RESOURCE_EXHAUSTED, "host budget has no victim");
    ReclaimLocked(victim);                   // Frees the host buffer and its accounting.
    index.erase(KeyId(victim->record.key));  // Retires discoverability.
    stats_blocks_evicted.fetch_add(1, std::memory_order_relaxed);
  }
  return OkStatus();
}

KvcStatus Session::OffloadLocked(const BlockPtr& block) {
  if (!block->gpu) return OkStatus();
  if (KvcStatus status = MakeHostRoomLocked(block->payload_bytes); status.code != KVC_OK)
    return status;
  std::vector<unsigned char> host(block->payload_bytes);
  cudaSetDevice(device_ordinal);
  if (cudaMemcpy(host.data(), block->gpu, block->payload_bytes, cudaMemcpyDeviceToHost) !=
      cudaSuccess) {
    return ErrorStatus(KVC_TRANSPORT_ERROR, "device-to-host offload copy failed");
  }
  block->pool->Give(block->gpu);
  block->gpu = nullptr;
  block->pool.reset();
  block->host = std::move(host);
  host_bytes.fetch_add(block->payload_bytes, std::memory_order_relaxed);
  stats_offloads.fetch_add(1, std::memory_order_relaxed);
  stats_offload_bytes.fetch_add(block->payload_bytes, std::memory_order_relaxed);
  return OkStatus();
}

KvcStatus Session::MakeGpuRoomLocked(uint64_t bytes) {
#if 1
#endif
  while (GpuUsedLocked() + bytes > gpu_budget) {
    // Write the least-recently-use committed GPU block back to the host.
    // Blocks pinned by read handles or in-flight transfers are skipped:
    // their storage must not move while others reference it.
    BlockPtr victim;
    for (auto& [_, block] : index) {
      if (!block->gpu || block->tx_private || block.use_count() != 1) continue;
      if (!victim || block->last_use < victim->last_use) victim = block;
    }
    if (!victim) return ErrorStatus(KVC_RESOURCE_EXHAUSTED, "gpu budget has no victim");
    if (KvcStatus status = OffloadLocked(victim); status.code != KVC_OK) return status;
  }
  return OkStatus();
}

bool Session::TryPromoteLocked(const BlockPtr& block) {
  if (block->gpu) return true;
  if (block->tx_private) return false;
  // A load is an access: make room like any allocation would, evicting
  // other unpinned LRU blocks, before restoring this block to the device.
  if (KvcStatus status = MakeGpuRoomLocked(block->payload_bytes); status.code != KVC_OK)
    return false;
  SlotPool* pool = PoolForLocked(block->payload_bytes);
  void* slot = pool->Take();
  if (!slot) return false;
  cudaSetDevice(device_ordinal);
  if (cudaMemcpy(slot, block->host.data(), block->payload_bytes, cudaMemcpyHostToDevice) !=
      cudaSuccess) {
    pool->Give(slot);
    return false;
  }
  block->gpu = slot;
  block->pool = pools.at(block->payload_bytes);
  block->host.clear();
  block->host.shrink_to_fit();
  host_bytes.fetch_sub(block->payload_bytes, std::memory_order_relaxed);
  stats_promotions.fetch_add(1, std::memory_order_relaxed);
  stats_promotion_bytes.fetch_add(block->payload_bytes, std::memory_order_relaxed);
  return true;
}

void Session::ReclaimLocked(const BlockPtr& block) { block->ReclaimStorage(); }

void Session::FinishTransferLocked(Transfer* transfer) {
  for (auto& block : transfer->blocks)
    if (block->pending) --block->pending;
  // Stop tracking a store transfer once it is terminal; commit consults the
  // transaction's pending list, which therefore never dangles.
  if (transfer->tx) {
    auto& pending_list = transfer->tx->transfers;
    for (size_t i = 0; i < pending_list.size(); ++i) {
      if (pending_list[i] == &transfer->header) {
        pending_list[i] = pending_list.back();
        pending_list.pop_back();
        break;
      }
    }
  }
  if (transfer->poisoned_tx && transfer->tx && transfer->final_code != KVC_OK) {
    // A failed accepted store poisons the transaction.
    if (transfer->tx->state != KVC_COMMITTED && transfer->tx->state != KVC_WRITE_ABORTED)
      AbortLocked(*this, transfer->tx);
  }
}
...
KvcStatus OpenSession(const KvcDescriptor* options, uint64_t timeout, KvcObject** out) {
  ...
  return Guard([&]() -> KvcStatus {
    ...
    *out = nullptr;
    ...
    // Options schema "kvc.cuda.options" v1, 24 canonical bytes:
    //   u64 gpu_budget  (0 = auto: min(free/4, 2 GiB) at configure time)
    //   u64 host_budget (0 = default 1 GiB)
    //   u32 device_ordinal
    ...
    if (options && options->schema.name.size != 0) {
      const KvcSchema& schema = options->schema;
      if (schema.name.size != 15 ||
          std::memcmp(schema.name.data, "kvc.cuda.options", 15) != 0 || schema.major != 1)
        return ErrorStatus(KVC_INVALID_ARGUMENT, "unknown provider options schema");
      if (options->canonical_payload.size == 0) {
        // Schema named but empty payload keeps the defaults.
      } else if (options->canonical_payload.size == 24) {
        const unsigned char* p = options->canonical_payload.data;
        gpu_budget = ReadLe64(p);
        host_budget = ReadLe64(p + 8);
        ordinal = static_cast<int>(ReadLe32(p + 16));
        if (ReadLe32(p + 20) != 0)
          return ErrorStatus(KVC_INVALID_ARGUMENT, "reserved options word must be zero");
        if (host_budget == 0) host_budget = kDefaultHostBudget;
      } else {
        return ErrorStatus(KVC_INVALID_ARGUMENT, "options payload must be 24 encoded bytes");
      }
    }

    int devices = 0;
    if (cudaGetDeviceCount(&devices) != cudaSuccess || devices <= 0)
      return ErrorStatus(KVC_UNAVAILABLE, "no CUDA device is available");
    if (ordinal < 0 || ordinal >= devices)
      return ErrorStatus(KVC_INVALID_ARGUMENT, "options name an unknown device ordinal");
    ...
    session->device_ordinal = ordinal;
    session->gpu_budget = gpu_budget;
    session->host_budget = host_budget;
...
KvcStatus SessionCapabilities(KvcObject* object, KvcCapabilities* out) {
  ...
  return Guard([&]() -> KvcStatus {
    ...
    out->memory_types = (1u << KVC_HOST) | (1u << KVC_PINNED_HOST) | (1u << KVC_DEVICE);
...
KvcStatus SessionConfigure(KvcObject* object, const KvcConfig* request, uint64_t timeout) {
  ...
  return Guard([&]() -> KvcStatus {
    ...
    // Validate and deep-copy the groups (same profile as the host provider).
    ...
    for (uint64_t g = 0; g < request->group_count; ++g) {
      ...
      if (row_bytes == 0)
        return ErrorStatus(KVC_INVALID_ARGUMENT, "a group needs a positive payload size");
      stored.payload_bytes = row_bytes * group.layer_count;
      smallest_block = std::min(smallest_block, stored.payload_bytes);
    ...
    // Initialize the device context this session will use.
    if (cudaSetDevice(session->device_ordinal) != cudaSuccess)
      return ErrorStatus(KVC_UNAVAILABLE, "cudaSetDevice failed");
    cudaStream_t stream = nullptr;
    if (cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking) != cudaSuccess)
      return ErrorStatus(KVC_UNAVAILABLE, "stream creation failed");
    uint64_t gpu_budget = session->gpu_budget;
    if (gpu_budget == 0) {
      size_t free_bytes = 0, total_bytes = 0;
      if (cudaMemGetInfo(&free_bytes, &total_bytes) != cudaSuccess) {
        cudaStreamDestroy(stream);
        return ErrorStatus(KVC_UNAVAILABLE, "device memory query failed");
      }
      gpu_budget = std::min(free_bytes / 4, kMaxAutoGpuBudget);
    }
    if (gpu_budget < smallest_block) {
      cudaStreamDestroy(stream);
      return ErrorStatus(KVC_RESOURCE_EXHAUSTED, "gpu budget cannot hold one block");
    }
...
KvcStatus SessionStats(KvcObject* object, KvcStatsRecord* out) {
  return Guard([&]() -> KvcStatus {
    ...
    for (auto& [_, pool] : session->pools) {
      allocations += pool->allocations.load(std::memory_order_relaxed);
      reuses += pool->reuses.load(std::memory_order_relaxed);
    }
    *out = KvcStatsRecord{};
    out->struct_size = sizeof(*out);
    ...
    out->gpu_allocations = allocations;
    out->gpu_reuses = reuses;
    out->offloads = session->stats_offloads.load(std::memory_order_relaxed);
    out->offload_bytes = session->stats_offload_bytes.load(std::memory_order_relaxed);
    out->promotions = session->stats_promotions.load(std::memory_order_relaxed);
    out->promotion_bytes = session->stats_promotion_bytes.load(std::memory_order_relaxed);
    out->gpu_bytes_in_use = session->GpuUsedLocked();
    out->gpu_budget = session->gpu_budget;
    out->host_bytes_in_use = session->host_bytes.load(std::memory_order_relaxed);
    out->host_budget = session->host_budget;
    return OkStatus();
  });
}
...
KvcStatus Lookup(KvcObject* object, const KvcLookupRequest* request, uint64_t timeout,
  ...
  return Guard([&]() -> KvcStatus {
    ...
    // Every candidate is evaluated independently; the longest fully
    // available candidate wins. Availability need not be monotonic.
...
KvcStatus BeginWrite(KvcObject* object, const KvcWriteRequest* request, uint64_t timeout,
  ...
  return Guard([&]() -> KvcStatus {
    ...
    *out = nullptr;
    ...
    // Reserve GPU budget for the whole manifest, evicting committed state
    // if necessary. Slots are allocated lazily as stores arrive.
    if (KvcStatus status = session->MakeGpuRoomLocked(bytes); status.code != KVC_OK)
      return status;

    for (const auto& record : tx->manifest) {
      auto block = std::make_shared<Block>();
      block->record = record;
      block->payload_bytes = session->FindGroup(record.group)->payload_bytes;
    ...
    // Nothing below can fail: install the reservation.
    for (const auto& block : write->tx->blocks)
      session->reserved_keys.insert(Session::KeyId(block->record.key));
    session->gpu_reserved += bytes;
    write->tx->reserved_remainder = bytes;
...
KvcStatus Commit(KvcObject* object, KvcObject* write_handle, uint64_t timeout,
  ...
  return Guard([&]() -> KvcStatus {
    ...
    // A failed accepted store has already aborted the transaction.
    ...
    for (Header* handle : write->tx->transfers) {
      auto* transfer = reinterpret_cast<Transfer*>(handle);
      if (ObserveTransferLocked(transfer) == KVC_PENDING)
        return ErrorStatus(KVC_BUSY, "stores of this transaction are still pending");
    }
    if (write->tx->state == KVC_WRITE_ABORTED)
      return ErrorStatus(KVC_ABORTED, "a store of this transaction failed");
    if (write->tx->reserved_remainder != 0)
      return ErrorStatus(KVC_INTERNAL, "coverage without allocation is inconsistent");
    ...
    // block references so published blocks are owned by the index alone and
    // become eligible for offloading and eviction.
    for (const auto& block : write->tx->blocks) {
      session->reserved_keys.erase(Session::KeyId(block->record.key));
      block->tx_private = false;
...
KvcStatus RemoveBlock(KvcObject* object, const KvcDigest* key, uint64_t timeout) {
  ...
  return Guard([&]() -> KvcStatus {
    ...
    std::lock_guard<std::mutex> lock(session->mutex);
    if (session->closing) return ErrorStatus(KVC_SHUTTING_DOWN, "session is closing");
    auto it = session->index.find(Session::KeyId(*key));
    if (it == session->index.end()) return OkStatus();  // Idempotent retirement.
    // If this session holds the only reference, reclaim now so the block's
    // destruction under this lock stays trivial; pinned generations keep
    // their storage until the last reader releases.
    if (it->second.use_count() == 1) session->ReclaimLocked(it->second);
    session->index.erase(it);
...
void ReleaseObject(KvcObject* object) {
  ...
  switch (header->kind) {
    ...
    case KVC_TRANSFER: {
      ...
      // Draining on release keeps the pending/retention invariants true for
      // callers that never poll or wait.
      DrainTransfer(transfer);
