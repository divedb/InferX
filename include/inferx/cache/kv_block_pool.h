...
struct KvLayout {
  ...
  /// \brief Returns the number of scalar elements one token occupies per
  ///        layer.
  int64_t ElementsPerToken() const { return entries_per_token * kv_heads * head_dim; }

  /// \brief Returns the number of bytes one token occupies per layer.
...
/// ordinary `Tensor`s rather than as a strided five-dimensional thing every
...
class KvBlockPool {
 public:
  ...
  /// view would just be indexed apart again at every use. The returned Tensors
  /// borrow the pool's allocation: the pool must outlive them.
  ///
  ...
  /// \param layer Layer index.
  /// \return      The cache tensor, or an error status for an invalid layer.
  /// \see KeyCache for the shape and layout.
  ...
  static int64_t BlockBytes(int64_t num_layers, int64_t block_size, const KvLayout& layout) {
...
class BlockTable {
 public:
  /// \brief Constructs a table whose blocks each hold `block_size` tokens.
  ///
  /// \param block_size Tokens per block; must match the pool's.
  ...
  ///
  /// \param pos   Logical token position.
  /// \param block Output: the block index holding the token.
  /// \param slot  Output: the slot of the token within the block.
