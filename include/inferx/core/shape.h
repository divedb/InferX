...
class Shape {
 public:
  ...
  /// \brief The container type holding the dimensions.
  ...
  /// \brief Returns an iterator to the first dimension.
  auto begin() const { return dims_.begin(); }
  /// \brief Returns an iterator past the last dimension.
  ...
  /// \brief Compares two shapes for equal dimensions.
  ...
  /// \brief Extends an absl hash to cover Shape, for use as a map key.
  template <typename H>
