...
/// \brief The class of device backing a buffer or kernel launch.
enum class DeviceKind : uint8_t {
  kCpu = 0,    ///< The single host address space.
  kCuda = 1,   ///< An NVIDIA GPU addressed via the CUDA runtime.
  kRocm = 2,   ///< An AMD GPU addressed via the ROCm runtime.
  kAscend = 3, ///< A Huawei Ascend NPU.
};
...
struct DeviceId {
  ...
  ///
  /// \param ordinal The CUDA runtime device ordinal.
  /// \return        The corresponding DeviceId.
  static constexpr DeviceId Cuda(int8_t ordinal) {
    return DeviceId(DeviceKind::kCuda, ordinal);
  }

  /// \brief The ROCm device with the given runtime `ordinal` (0..N-1).
  ///
  /// \param ordinal The ROCm runtime device ordinal.
  /// \return        The corresponding DeviceId.
  static constexpr DeviceId Rocm(int8_t ordinal) {
    return DeviceId(DeviceKind::kRocm, ordinal);
  }

  /// \brief The Ascend NPU with the given `ordinal`.
  ///
  /// \param ordinal The Ascend device ordinal.
  /// \return        The corresponding DeviceId.
  static constexpr DeviceId Ascend(int8_t ordinal) {
    return DeviceId(DeviceKind::kAscend, ordinal);
  }

  /// \brief Names a device of `kind` at `ordinal`, normalizing CPU to `Cpu()`.
  ///
  /// \param kind   The device class.
  /// \param ordinal The device ordinal; ignored for CPU.
  /// \return       The corresponding DeviceId.
  ...
  /// \brief True if this device is any kind of accelerator (non-host).
  constexpr bool IsAccelerator() const { return !IsCpu(); }

  /// \brief Compares two device ids for equality (same kind and ordinal).
  friend constexpr bool operator==(const DeviceId& a, const DeviceId& b) {
    return a.kind == b.kind && a.index == b.index;
  }

  /// \brief Compares two device ids for inequality.
  friend constexpr bool operator!=(const DeviceId& a, const DeviceId& b) {
    return !(a == b);
  }
  ...
  /// \brief Extends an absl hash to cover DeviceId, for use as a map key.
  template <typename H>
 ...
 private:
  /// \brief Constructs a (kind, ordinal) pair.
  ///
