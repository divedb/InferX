...
/// \brief Alias for absl::Status, the engine's error type.
using Status = absl::Status;

/// \brief Alias for absl::StatusOr<T>, a value-or-error result.
template <typename T>
...
/// \brief Evaluates `expr`, which must return a `Status`. If the result is not
///        OK, returns it from the current function.
///
/// \param expr An expression that returns a `Status`.
