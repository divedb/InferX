...
class BumpArena {
 public:
  ...
  ///              allocation fails.
  StatusOr<void*> Allocate(size_t bytes) { return Allocate(bytes, kTensorAlignment); }

  /// \brief Allocates a Tensor over arena memory with the specified data type
  ///        and shape.
  ///
  /// The returned Tensor borrows the arena's bytes: it keeps no reference, so
  /// it is valid only until the arena is Reset() or destroyed.
  ///
  /// \param dtype The data type of the Tensor to allocate.
  ...
  StatusOr<Tensor> AllocateTensor(DataType dtype, const Shape& shape);

  /// \brief Releases everything at once.
  ///
  /// Any Tensor still pointing into the arena dangles afterwards; this is only
  /// for teardown and for reuse between tests. Deliberately does not rewind
  /// the high-water mark: the point of that number is to size the arena across
  /// a whole run, and a per-step Reset() would erase exactly the peak it is
  /// meant to record.
