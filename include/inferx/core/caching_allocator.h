...
class CachingAllocator final : public Allocator {
 public:
  ...
  /// \brief Allocates `bytes` aligned to at least `alignment` from the region.
  ///
  ...
  /// Re-exposes the base's one-argument Allocate: declaring the override above
  /// would otherwise hide it for anyone holding a CachingAllocator by its
  /// concrete type.
  using Allocator::Allocate;

  /// \brief Allocates a Tensor with the specified TensorSpec.
  ///
  /// The returned Tensor borrows the allocator's bytes: it keeps no reference,
  /// so the block returns to the free list only through an explicit
  /// Deallocate() of its data pointer.
  ///
  /// \param spec The TensorSpec describing the desired data type and shape of
  ///             the tensor.
  /// \return     A StatusOr containing the allocated Tensor or an error
  ///             status.
  StatusOr<Tensor> AllocateTensor(const TensorSpec& spec);
  ...
  /// \brief Returns a snapshot of the allocator's statistics.
  ///
  /// \return The current Stats.
  Stats GetStats() const;
  /// \brief Returns the capacity of the managed region in bytes.
  ...
  /// \brief Returns the device the region lives on.
  DeviceId Device() const override { return device_; }
  /// \brief Returns the allocator's diagnostic name.
